  ...
  <div class="layout-tagView w-full bg-$color-white">
    <el-scrollbar class="h-full w-full pl-4 pr-4" ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-tagView-ul h-full flex items-center">
        <li v-for="item in visitedViews" :key="item.name" :class="{
          'is-active text-$color-white bg-$el-color-primary': path === item.path,
        }" @click="onHandleClickSwitchRouter(item)" @contextmenu="onHandleContextMenu($event, item)"
          ...
          <!-- <div v-if="path === item.path"
            class="refresh-wrapper text-$color-text-primary rounded-full ml-1 w-4 h-4 flex items-center justify-center"
            @click.stop="onHandleRefreshTag(item)">
            <svg-icon name="refresh"></svg-icon>
          </div> -->
          <div v-if="visitedViews.length > 1 && !item.meta.isAffix" :class="{ 'ml-1': path !== item.path }"

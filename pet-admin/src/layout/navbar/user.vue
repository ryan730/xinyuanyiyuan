<template>
  <div class="layout-user h-full select-none flex items-center">
    <el-dropdown type="primary" :show-timeout="70" :hide-timeout="50" trigger="hover"
      class="dropdown h-full flex items-center pr-2" @command="onHandleClickCommand">
      <div class="icon-wrapper pl-2.5 pr-2.5 h-full flex justify-center items-center cursor-pointer">
        ...
        {{ userInfo.name }}<el-icon class="ml-3">
          <ArrowDown />
        </el-icon>
      </div>
      <template #dropdown>

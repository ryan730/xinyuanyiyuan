      ...
      <div class="p-2 bg-white">
        <div class="table-header flex">
          <div class="table-header-title w-40 text-left">{{
            i18nRouter('distributionPurseList')
          }}</div>
        ...
        <el-table :data="userTable" ref="multipleTableRef" border @selection-change="handleSelectionChange">
          ...
          <el-table-column show-overflow-tooltip label="用户名" prop="nick_name" />
          <el-table-column show-overflow-tooltip label="提取金额" prop="withdraw" />
          <el-table-column show-overflow-tooltip label="订单总收⼊（分）" prop="income" />
...
const getUserList = async (date: any = {}) => {
  ...
  const res = (await statisticsService.purse({ page: searchForm.pageNum, ...timer })) as any;

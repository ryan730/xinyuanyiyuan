    ...
    <div class="user-table w-[calc(100%-15rem)] ml-5" v-loading="searchForm.loading">
      <div class="p-2 bg-white">
        ...
        <el-table :data="userTable" ref="multipleTableRef" border @selection-change="handleSelectionChange">
          ...
          <el-table-column width="100" show-overflow-tooltip label="商品id" prop="product_id" />
          <el-table-column show-overflow-tooltip label="⽤⼾openid" prop="openid" />
          <el-table-column show-overflow-tooltip label="订单⾦额" prop="amount" />
          <el-table-column show-overflow-tooltip label="订单状态" prop="pay_status" :formatter="typeFormatter" />
          <el-table-column class-name="colum-elm" show-overflow-tooltip label="下单时间" prop="created_at" />
        </el-table>
...
const typeFormatter = (row: any, column: any) => {
  return statusPay[row?.pay_status];
};
...
const getUserList = async (date: any = {}) => {
  ...
  const res = (await ListService.orderList({ page: searchForm.pageNum, ...timer })) as any;

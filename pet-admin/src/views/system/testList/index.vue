  ...
  <div class="user-page w-full flex h-full">
    <div class="user-table w-[calc(100%-15rem)] ml-5" v-loading="searchForm.loading">
      <div class="p-2 bg-white">
        ...
        <el-table :class="'hover-row'" :data="userTable" ref="multipleTableRef" border @selection-change="handleSelectionChange" @row-click="handleRowClick">
          <el-table-column type="selection" width="60" :selectable="canBeSelect" />

          <el-table-column width="80" show-overflow-tooltip label="测评id" prop="id"/>
          <el-table-column show-overflow-tooltip label="测评标题" prop="title"/>
          <el-table-column show-overflow-tooltip label="⼩标题" prop="sub_title"/>
          <el-table-column show-overflow-tooltip label="题图" prop="type"/>
...
const getUserList = async () => {
  ...
  const res = (await ListService.testList({ page: searchForm.pageNum, pid: route.params.id })) as any;

  console.log('ListService-----', route,searchForm.pageNum, res);
...
const handleRowClick = (row: any, column: any, event: MouseEvent)=> {
      ...
      router.push(`/system/topicList/${row.id}`); // 跳转到商品详情页，假设URL模式是'/product/detail/:id'
}

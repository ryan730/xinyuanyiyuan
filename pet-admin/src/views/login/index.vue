  ...
  <div class="login-page w-full h-full bg-cover select-none">
    <div class="login-content p-5 absolute top-2/4 left-2/4 bg-white rounded overflow-hidden">
      <div class="login-content-main w-4/5 h-full mx-auto my-0">
        <h4 class="text-dark-300 font-bold pt-10 pb-10 tracking-wider text-center whitespace-normal z-10 relative h-auto transition-all">{{ fixedStore.loginTitle }} 管理后台</h4>
...
const onSubmit = () => {
  proxy.$refs.loginForm.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        let raw = toRaw(form);
        const { code, success, data: userInfo, message } = (await LoginService.login({
          name: encodeURIComponent(raw.account),
          pass: encodeURIComponent(raw.password),
        })) as any;

        if (code === 200 && success) {
          console.log('login:res-----',code,success,userInfo);

    ...
    <div class="section_1 flex-col">
      <div class="group_2 flex-col">
        <div class="group_3 flex-row justify-between">
          <img class="image_2" style="opacity: 0" referrerpolicy="no-referrer" :src="require('../assets/images/qrback.png')" @click="onClickLeft" />
          <span class="text_2">生成推广二维码</span>
        </div>
        <!-- <div class="text-wrapper_1 flex-row">
          <span class="text_3">生成二维码</span>
        </div> -->
        <div class="group_4 flex-row">
          <div class="image-wrapper_1 flex-col">
            <div id="qrcode" v-if="!realImag"></div>
            <img class="realImag" :src="realImag" v-if="realImag" referrerpolicy="no-referrer" style="width: 100%; height: 100%" alt />
          </div>
        </div>
        <div class="group_5 flex-row">
          <div class="text-wrapper_2 flex-col" @click="handleSave">
            <span class="text_4 no-select">保存二维码为图片</span>
          </div>
        </div>
      </div>
    </div>
    ...
    onMounted(async () => {
      ...
      if (shareURLRes?.code == 1) {
        ...
        if (url_link) {
          ...
          // 长按功能
          let cav = document.querySelector("#qrcode canvas");
          cav.style.display = "none";
          realImag.value = cav.toDataURL("image/png");

          console.log("canvas===", cav.style.display, realImag);
        } else {

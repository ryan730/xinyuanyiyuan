...
function calTooltipYAxisData(point, series, opts, config, eachSpacing) {
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    item = opts.yAxis.data[i].formatter ? opts.yAxis.data[i].formatter(item, i, opts) : item.toFixed(0);
...
function drawPointShape(points, color, shape, context, opts) {
  ...
  if (opts.dataPointShapeType == 'hollow') {
    ...
    context.setLineWidth(2 * opts.pix);// 表格像素 mark
  } else {
...
function drawPointText(points, series, config, context, opts) {
  ...
  points.forEach(function(item, index) {
    if (item !== null) {
      ...
      formatVal = formatVal+series.chhlabels;
      context.fillText(String(formatVal), item.x, item.y - 8 + textOffset * opts.pix);
      context.closePath();
      context.stroke();
...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
  ...
  angleList.forEach(function(angle, index) {
    ...
    if(opts.categories.length % 2 == 0 && index == opts.categories.length/2){
      startY = startY + 2;
    }
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config._xAxisTextAngle_ === 0) {
      newCategories.forEach(function(item, index) {
        ...
        // chh 最底下 文本
        context.fillText(String(xitem), xAxisPoints[index] + offset, startY+20 + xAxisFontSize + (config.xAxisHeight - scrollHeight - xAxisFontSize) / 2);
        context.closePath();
        context.stroke();
      });
    } else {
        ...
        rangesFormat.forEach(function(item, index) {
          ...
          if (yAxisWidth.position == 'left') {
            ...
            if (yData.calibration == true) {
              ...
              context.lineTo(tStartLeft - 3 * opts.pix, pos); // 刻度文本mark
              gapwidth += 3 * opts.pix;
            }
            //画文字
            switch (textAlign) {
            ...
            // 柱状图 Y 轴 文案
            item = item + series[0].chhlabels;
            context.fillText(String(item), tmpstrat-5, pos + yAxisFontSize / 2 - 3 * opts.pix); /// 设置Y轴坐标
...
function drawRadarDataPoints(series, opts, config, context) {
  ...
  radarDataPoints.forEach(function(eachSeries, seriesIndex) {
    ...
    if(eachSeries.color == '#FFA000'){
      radarOption.opacity = 0;
      var fillcolor = hexToRgb(eachSeries.color, 0);
    }
 ...
 /// 雷达图自定义mark
  ...
  if(radarOption.axisLabel === true){
    ...
    for (var i = 0; i < radarOption.gridCount + 1; i++) {
      ...
      // label = radarOption.axisLabels[i]
      const rlen = radius - (radius - i * (stepLength-1) + fontSize / 2) -8;
      var pos = {
        x: (rlen + config.radarLabelTextMargin * opts.pix) * Math.cos(360),
        y: (rlen + config.radarLabelTextMargin * opts.pix) * Math.sin(360)
      };
      var posRelativeCanvas = convertCoordinateOrigin(pos.x, pos.y, centerPosition);
      var startX = posRelativeCanvas.x;
      var startY = posRelativeCanvas.y;
      ...
      // console.log(pos,Math.sin(10),Math.cos(30) )
      context.fillText(String(label), startX , startY+10);
    }
  }
    ...
    case 'column':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context); // 画Y轴刻度
          if (opts.enableMarkLine !== false && process === 1) {
            drawMarkLine(opts, config, context);
          }
          // drawLegend(opts.series, opts, config, context, opts.chartData);
